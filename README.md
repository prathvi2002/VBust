...
- There’s currently a bug in VBust that causes a `TypeError` and the tools exits when using the `--proxy` option. As a temporary workaround, you can bypass this issue by using the `--success` flag with a non-existent response code along with the `--proxy` option. Example: `--proxy http://127.0.0.1:9090 --success 999`. This is a hacky workaround, I haven’t had time to properly debug and fix this yet, so this should help keep things moving in the meantime.
